...
* 레이아웃: 2~3단 구성 권장 (좌측 텍스트, 우측 제품 / 상단 브랜드, 하단 CTA 등)`;
    ...
    if (logoImage || productImage) {
      console.log('이미지 편집 API 호출 준비...');
      ...
      try {
        ...
        // 이미지 데이터를 임시 파일로 저장
        if (logoImage) {
          // Base64 접두사 제거 및 이미지 데이터만 추출
          const logoData = logoImage.split(',')[1] || logoImage;
          const logoBuffer = Buffer.from(logoData, 'base64');
          const logoPath = path.join(tempDir, 'logo.png');
          fs.writeFileSync(logoPath, logoBuffer);
          tempImageFiles.push(logoPath);
        }
        
        if (productImage) {
          // Base64 접두사 제거 및 이미지 데이터만 추출
          const productData = productImage.split(',')[1] || productImage;
          const productBuffer = Buffer.from(productData, 'base64');
          const productPath = path.join(tempDir, 'product.png');
          fs.writeFileSync(productPath, productBuffer);
          tempImageFiles.push(productPath);
        }
        ...
        // 이미지 파일 추가
        tempImageFiles.forEach((filePath, index) => {
          const fileStream = fs.createReadStream(filePath);
          formData.append(`image[${index}]`, fileStream);
        });
        ...
        // 임시 파일 정리
        tempImageFiles.forEach(filePath => {
          try {
        ...
        if (!apiResponse.ok) {
          throw new Error(`API 호출 실패: ${JSON.stringify(responseData)}`);
        }
        
        imageResponse = responseData;
      } catch (editError) {
        console.error('이미지 편집 API 호출 실패:', editError);
        
        // 이미지 편집 API가 실패하면 기본 생성 API 사용
        console.log('기본 이미지 생성 API로 대체하여 시도합니다...');
